...
export const CommentDisplay = ({ apiData, searchResultJson }) => {
  ...
  return (
    <div className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-2xl transition-shadow duration-300">
      ...
      {/* 3. YouTube APIから取得したコメントの表示エリア (修正済み) */}
        ...
        <div className="space-y-4">
          {apiData.comments.map((comment, index) => (
            <pre 
              ...
              className="bg-gray-100 dark:bg-gray-900 p-4 rounded-lg text-xs md:text-sm text-gray-800 dark:text-gray-200 shadow-inner overflow-x-auto whitespace-pre-wrap font-mono"
            >
              {formatJsonForDisplay(comment)}
            </pre>
          ))}
        </div>
      ) : (
